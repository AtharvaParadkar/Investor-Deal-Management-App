            ...
            children: [
              ...
              SliderTheme(
                ...
                child: RangeSlider(
                  ...
                  onChanged: (values) => context.read<FilterBloc>()
                      .add(ROIRangeChanged(values.start, values.end)))),
              const SizedBox(height: AppSpacing.xl),
              Text(AppStrings.riskLevel,
                style: AppTextStyles.subheading.copyWith(fontSize: 14)),
              const SizedBox(height: AppSpacing.sm),
              Row(
                children: RiskLevel.values.map((level) {
                  final isSelected = criteria.selectedRiskLevels.contains(level);
                  ...
                  return Padding(
                    ...
                    child: AppChip(
                      ...
                      onTap: () => context.read<FilterBloc>()
                          .add(RiskLevelToggled(level))));
                }).toList()),
              const SizedBox(height: AppSpacing.xl),
              Text(AppStrings.industry,
                style: AppTextStyles.subheading.copyWith(fontSize: 14)),
              const SizedBox(height: AppSpacing.sm),
              Wrap(
                ...
                children: industries.map((industry) {
                  final isSelected = criteria.selectedIndustries.contains(industry);
                  return AppChip(
                    label: industry,
                    isSelected: isSelected,
                    onTap: () => context.read<FilterBloc>()
